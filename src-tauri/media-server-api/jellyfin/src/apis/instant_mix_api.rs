...
#[deprecated]
pub async fn get_instant_mix_from_artists2(configuration: &configuration::Configuration, params: GetInstantMixFromArtists2Params) -> Result<models::BaseItemDtoQueryResult, Error<GetInstantMixFromArtists2Error>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetInstantMixFromArtists2Error> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
