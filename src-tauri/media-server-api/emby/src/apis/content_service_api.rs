...
/// struct for passing parameters to the method [`get_users_by_userid_homesections`]
#[derive(Clone, Debug)]
pub struct GetUsersByUseridHomesectionsParams {
    /// User Id
    pub user_id: String
}

/// struct for passing parameters to the method [`get_users_by_userid_sections_by_sectionid_items`]
#[derive(Clone, Debug)]
pub struct GetUsersByUseridSectionsBySectionidItemsParams {
...
/// struct for typed errors of method [`get_users_by_userid_homesections`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetUsersByUseridHomesectionsError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
/// struct for typed errors of method [`get_users_by_userid_sections_by_sectionid_items`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetUsersByUseridSectionsBySectionidItemsError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_users_by_userid_homesections(configuration: &configuration::Configuration, params: GetUsersByUseridHomesectionsParams) -> Result<Vec<models::ContentSection>, Error<GetUsersByUseridHomesectionsError>> {

    let uri_str = format!("{}/Users/{UserId}/HomeSections", configuration.base_path, UserId=crate::apis::urlencode(params.user_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::ContentSection&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::ContentSection&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetUsersByUseridHomesectionsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Requires authentication as user
pub async fn get_users_by_userid_sections_by_sectionid_items(configuration: &configuration::Configuration, params: GetUsersByUseridSectionsBySectionidItemsParams) -> Result<models::QueryResultBaseItemDto, Error<GetUsersByUseridSectionsBySectionidItemsError>> {

    let uri_str = format!("{}/Users/{UserId}/Sections/{SectionId}/Items", configuration.base_path, SectionId=crate::apis::urlencode(params.section_id), UserId=crate::apis::urlencode(params.user_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetUsersByUseridSectionsBySectionidItemsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
