...
/// struct for passing parameters to the method [`get_users_by_userid_items`]
#[derive(Clone, Debug)]
pub struct GetUsersByUseridItemsParams {
    /// User Id
    pub user_id: String,
    /// Artist or AlbumArtist
    pub artist_type: Option<String>,
...
/// struct for passing parameters to the method [`get_users_by_userid_items_resume`]
#[derive(Clone, Debug)]
...
/// struct for typed errors of method [`get_users_by_userid_items`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetUsersByUseridItemsError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
/// struct for typed errors of method [`get_users_by_userid_items_resume`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetUsersByUseridItemsResumeError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_items(configuration: &configuration::Configuration, params: GetItemsParams) -> Result<models::QueryResultBaseItemDto, Error<GetItemsError>> {
...
/// Requires authentication as user
pub async fn get_users_by_userid_items(configuration: &configuration::Configuration, params: GetUsersByUseridItemsParams) -> Result<models::QueryResultBaseItemDto, Error<GetUsersByUseridItemsError>> {

    let uri_str = format!("{}/Users/{UserId}/Items", configuration.base_path, UserId=crate::apis::urlencode(params.user_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetUsersByUseridItemsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Requires authentication as user
pub async fn get_users_by_userid_items_resume(configuration: &configuration::Configuration, params: GetUsersByUseridItemsResumeParams) -> Result<models::QueryResultBaseItemDto, Error<GetUsersByUseridItemsResumeError>> {

    let uri_str = format!("{}/Users/{UserId}/Items/Resume", configuration.base_path, UserId=crate::apis::urlencode(params.user_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetUsersByUseridItemsResumeError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
