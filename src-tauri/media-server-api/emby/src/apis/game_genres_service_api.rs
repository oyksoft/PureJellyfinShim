...
/// struct for passing parameters to the method [`get_gamegenres`]
#[derive(Clone, Debug)]
pub struct GetGamegenresParams {
...
/// struct for passing parameters to the method [`get_gamegenres_by_name`]
#[derive(Clone, Debug)]
pub struct GetGamegenresByNameParams {
    /// The genre name
    pub name: String,
...
/// struct for typed errors of method [`get_gamegenres`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetGamegenresError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
/// struct for typed errors of method [`get_gamegenres_by_name`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetGamegenresByNameError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_gamegenres(configuration: &configuration::Configuration, params: GetGamegenresParams) -> Result<models::QueryResultBaseItemDto, Error<GetGamegenresError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetGamegenresError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Requires authentication as user
pub async fn get_gamegenres_by_name(configuration: &configuration::Configuration, params: GetGamegenresByNameParams) -> Result<models::BaseItemDto, Error<GetGamegenresByNameError>> {

    let uri_str = format!("{}/GameGenres/{Name}", configuration.base_path, Name=crate::apis::urlencode(params.name));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetGamegenresByNameError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
