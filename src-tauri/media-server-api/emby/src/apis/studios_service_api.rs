...
/// struct for passing parameters to the method [`get_studios_by_name`]
#[derive(Clone, Debug)]
pub struct GetStudiosByNameParams {
    /// The studio name
    pub name: String,
...
/// struct for typed errors of method [`get_studios_by_name`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetStudiosByNameError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_studios(configuration: &configuration::Configuration, params: GetStudiosParams) -> Result<models::QueryResultBaseItemDto, Error<GetStudiosError>> {
...
/// Requires authentication as user
pub async fn get_studios_by_name(configuration: &configuration::Configuration, params: GetStudiosByNameParams) -> Result<models::BaseItemDto, Error<GetStudiosByNameError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetStudiosByNameError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
