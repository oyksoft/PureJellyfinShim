...
/// struct for passing parameters to the method [`get_persons_by_name`]
#[derive(Clone, Debug)]
pub struct GetPersonsByNameParams {
    /// The person name
    pub name: String,
...
/// struct for typed errors of method [`get_persons_by_name`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPersonsByNameError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_persons(configuration: &configuration::Configuration, params: GetPersonsParams) -> Result<models::QueryResultBaseItemDto, Error<GetPersonsError>> {
...
/// Requires authentication as user
pub async fn get_persons_by_name(configuration: &configuration::Configuration, params: GetPersonsByNameParams) -> Result<models::BaseItemDto, Error<GetPersonsByNameError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetPersonsByNameError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
