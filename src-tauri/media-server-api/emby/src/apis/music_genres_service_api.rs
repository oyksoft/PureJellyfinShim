...
/// struct for passing parameters to the method [`get_musicgenres`]
#[derive(Clone, Debug)]
pub struct GetMusicgenresParams {
...
/// struct for passing parameters to the method [`get_musicgenres_by_name`]
#[derive(Clone, Debug)]
pub struct GetMusicgenresByNameParams {
    /// The genre name
    pub name: String,
...
/// struct for typed errors of method [`get_musicgenres`]
...
/// struct for typed errors of method [`get_musicgenres_by_name`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetMusicgenresByNameError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_musicgenres(configuration: &configuration::Configuration, params: GetMusicgenresParams) -> Result<models::QueryResultBaseItemDto, Error<GetMusicgenresError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetMusicgenresError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Requires authentication as user
pub async fn get_musicgenres_by_name(configuration: &configuration::Configuration, params: GetMusicgenresByNameParams) -> Result<models::BaseItemDto, Error<GetMusicgenresByNameError>> {

    let uri_str = format!("{}/MusicGenres/{Name}", configuration.base_path, Name=crate::apis::urlencode(params.name));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetMusicgenresByNameError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
