...
/// struct for passing parameters to the method [`get_genres_by_name`]
#[derive(Clone, Debug)]
pub struct GetGenresByNameParams {
    /// The genre name
    pub name: String,
...
/// struct for typed errors of method [`get_genres_by_name`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetGenresByNameError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_genres(configuration: &configuration::Configuration, params: GetGenresParams) -> Result<models::QueryResultBaseItemDto, Error<GetGenresError>> {
...
/// Requires authentication as user
pub async fn get_genres_by_name(configuration: &configuration::Configuration, params: GetGenresByNameParams) -> Result<models::BaseItemDto, Error<GetGenresByNameError>> {

    let uri_str = format!("{}/Genres/{Name}", configuration.base_path, Name=crate::apis::urlencode(params.name));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetGenresByNameError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
