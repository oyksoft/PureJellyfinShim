...
/// struct for passing parameters to the method [`get_users_by_userid_suggestions`]
#[derive(Clone, Debug)]
pub struct GetUsersByUseridSuggestionsParams {
    /// User Id
    pub user_id: String,
...
/// struct for typed errors of method [`get_users_by_userid_suggestions`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetUsersByUseridSuggestionsError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_users_by_userid_suggestions(configuration: &configuration::Configuration, params: GetUsersByUseridSuggestionsParams) -> Result<models::QueryResultBaseItemDto, Error<GetUsersByUseridSuggestionsError>> {

    let uri_str = format!("{}/Users/{UserId}/Suggestions", configuration.base_path, UserId=crate::apis::urlencode(params.user_id));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetUsersByUseridSuggestionsError> = serde_json::from_str(&content).ok();
