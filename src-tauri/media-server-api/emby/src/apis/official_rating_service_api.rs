...
/// struct for passing parameters to the method [`get_officialratings`]
#[derive(Clone, Debug)]
pub struct GetOfficialratingsParams {
...
/// struct for typed errors of method [`get_officialratings`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetOfficialratingsError {
    Status400(),
    Status401(),
    Status403(),
    Status404(),
...
pub async fn get_officialratings(configuration: &configuration::Configuration, params: GetOfficialratingsParams) -> Result<models::QueryResultUserLibraryOfficialRatingItem, Error<GetOfficialratingsError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::QueryResultUserLibraryOfficialRatingItem`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::QueryResultUserLibraryOfficialRatingItem`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetOfficialratingsError> = serde_json::from_str(&content).ok();
