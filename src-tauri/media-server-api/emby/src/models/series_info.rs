/*
 * Emby Server REST API
 *
 * Explore the Emby Server API
 *
 * The version of the OpenAPI document: 4.9.5.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct SeriesInfo {
    #[serde(rename = "EpisodeAirDate", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub episode_air_date: Option<Option<chrono::DateTime<chrono::FixedOffset>>>,
    #[serde(rename = "DisplayOrder", skip_serializing_if = "Option::is_none")]
    pub display_order: Option<models::SeriesDisplayOrder>,
    /// The name.
    #[serde(rename = "Name", skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    #[serde(rename = "Path", skip_serializing_if = "Option::is_none")]
    pub path: Option<String>,
    /// The metadata language.
    #[serde(rename = "MetadataLanguage", skip_serializing_if = "Option::is_none")]
    pub metadata_language: Option<String>,
    /// The metadata country code.
    #[serde(rename = "MetadataCountryCode", skip_serializing_if = "Option::is_none")]
    pub metadata_country_code: Option<String>,
    #[serde(rename = "MetadataLanguages", skip_serializing_if = "Option::is_none")]
    pub metadata_languages: Option<Vec<models::GlobalizationCultureDto>>,
    #[serde(rename = "ProviderIds", skip_serializing_if = "Option::is_none")]
    pub provider_ids: Option<std::collections::HashMap<String, String>>,
    /// The year.
    #[serde(rename = "Year", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub year: Option<Option<i32>>,
    #[serde(rename = "IndexNumber", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub index_number: Option<Option<i32>>,
    #[serde(rename = "ParentIndexNumber", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub parent_index_number: Option<Option<i32>>,
    #[serde(rename = "PremiereDate", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub premiere_date: Option<Option<chrono::DateTime<chrono::FixedOffset>>>,
    #[serde(rename = "IsAutomated", skip_serializing_if = "Option::is_none")]
    pub is_automated: Option<bool>,
    #[serde(rename = "EnableAdultMetadata", skip_serializing_if = "Option::is_none")]
    pub enable_adult_metadata: Option<bool>,
}

impl SeriesInfo {
    pub fn new() -> SeriesInfo {
        SeriesInfo {
            episode_air_date: None,
            display_order: None,
            name: None,
            path: None,
            metadata_language: None,
            metadata_country_code: None,
            metadata_languages: None,
            provider_ids: None,
            year: None,
            index_number: None,
            parent_index_number: None,
            premiere_date: None,
            is_automated: None,
            enable_adult_metadata: None,
        }
    }
}

